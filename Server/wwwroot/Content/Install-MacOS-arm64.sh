...
ETag=$(curl --head $HostName/Content/CoreConnect-MacOS-arm64.zip | grep -i "etag" | cut -d' ' -f 2)
...
if [ -z "$UpdatePackagePath" ]; then
    ...
    curl $HostName/Content/CoreConnect-MacOS-arm64.zip --output $InstallDir/CoreConnect-MacOS-arm64.zip
else
    echo  "Copying install files..." >> /tmp/CoreConnect_Install.log
    cp "$UpdatePackagePath" $InstallDir/CoreConnect-MacOS-arm64.zip
    rm -f "$UpdatePackagePath"
fi

unzip -o $InstallDir/CoreConnect-MacOS-arm64.zip -d $InstallDir
rm -f $InstallDir/CoreConnect-MacOS-arm64.zip
...
curl --head $HostName/Content/CoreConnect-MacOS-arm64.zip | grep -i "etag" | cut -d' ' -f 2 > $InstallDir/etag.txt
